...
const Accounting = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 1. Fetch Expenses (Money Out)
        const expensesRes = await fetch(`${API_URL}/api/expenses/`, { 
            headers: { 'Authorization': `Bearer ${token}` } 
        });

        // 2. Fetch Bookings to get Payments (Money In)
        ...
        if (expensesRes.ok && bookingsRes.ok) {
            ...
            // We extract payments from inside each booking object
            let creditTx = [];
            bookings.forEach(b => {
  ...
  // Simple Chart Data (Last 10 transactions for visual)
  ...
  return (
    <div className="p-4 md:p-8 bg-slate-50 min-h-full font-sans animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
        ...
        <div className="flex flex-wrap gap-4 w-full md:w-auto">
            <button onClick={() => window.print()} className="flex-1 md:flex-none bg-white border border-slate-200 text-slate-600 px-4 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-slate-50 flex items-center justify-center gap-2 transition-colors">
      ...
      <div className="bg-slate-900 rounded-[30px] p-8 mb-8 text-white relative overflow-hidden shadow-xl shadow-slate-200/50">
        ...
        <div className="flex items-end gap-2 h-32 w-full">
            {recentTx.map((t, i) => {
             ...
             {recentTx.length === 0 && <p className="text-slate-500 text-xs w-full text-center italic">No transactions found.</p>}
        </div>
