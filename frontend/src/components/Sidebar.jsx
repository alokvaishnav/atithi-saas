      ...
      `}>
        ...
        <div className="p-8 border-b border-white/5 flex items-center justify-between bg-slate-950">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xl shadow-lg text-white ${isSaaSAdmin ? 'bg-purple-600 shadow-purple-500/20' : 'bg-blue-600 shadow-blue-500/20'}`}>
              {isSaaSAdmin ? 'S' : (hotelName ? hotelName.charAt(0).toUpperCase() : 'A')}
            </div>
            <div className="overflow-hidden">
              <h1 className="text-sm font-black text-white italic tracking-tighter leading-none uppercase truncate max-w-[140px]">
                {isSaaSAdmin ? "SaaS Core" : (hotelName || "Loading...")}
              </h1>
              <p className={`text-[9px] font-black uppercase tracking-[0.2em] mt-1 ${isSaaSAdmin ? 'text-purple-400' : 'text-blue-400'}`}>
                {isSaaSAdmin ? "System CEO" : platformInfo.appName}
              </p>
            </div>
          </div>
        ...
        <div className="p-6 border-t border-white/5 bg-slate-900/50 backdrop-blur-md sticky bottom-0">
          <div className="px-4 py-3 mb-4 bg-white/5 rounded-2xl border border-white/10 flex items-center gap-3">
              <div className="w-8 h-8 bg-slate-800 rounded-lg flex items-center justify-center">
                <ShieldCheck size={16} className={isSaaSAdmin ? "text-purple-400" : "text-blue-400"}/>
              </div>
              <div className="overflow-hidden">
